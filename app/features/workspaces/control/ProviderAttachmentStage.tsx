...
import { beginWorkspaceOAuth, getGitHubRepositories, getVercelServices } from "@/app/components/handlers/connections";
import { attachVercelService } from "@/app/components/handlers/services";
import { attachGitHubSource } from "@/app/components/handlers/workspaces";
...
type ProviderAttachmentStageProps = {
  accountID: string;
  workspaceID: string;
  provider: Provider;
...
const providerCopy = {
  github: {
    ...
    description: "Choose the repository that supplies this workspace. InfraMap verifies access again before binding it.",
    mark: "GH",
...
export function ProviderAttachmentStage({ accountID, workspaceID, provider, role, resource, connections, connectionsLoading, connectionsError, onAttached }: ProviderAttachmentStageProps) {
  ...
  useEffect(() => {
    ...
    void Promise.resolve().then(async () => {
      ...
      try {
        ...
        if (active) {
          setError(cause instanceof Error ? cause.message : `${copy.title.replace("Connect ", "")} resources could not be loaded.`);
          setErrorCode(cause instanceof ApiError && typeof cause.payload.code === "string" ? cause.payload.code : "");
        }
      } finally {
    ...
    return () => { active = false; };
  }, [accountID, copy.title, provider, reloadKey, resource, selectedConnection]);
  ...
  async function reconnectProvider() {
    ...
    await beginWorkspaceOAuth(accountID, provider, returnTo);
  }
  ...
  async function attachResource() {
    ...
    try {
      const attached = provider === "github"
        ? await attachGitHubSource(accountID, workspaceID, selectedConnection.id, Number(selectedID))
        : await attachVercelService(accountID, workspaceID, selectedConnection.id, selectedID);
      onAttached(attached);
    } catch (cause) {
    ...
    <section id={copy.sectionID} className={styles.section}>
      ...
      {connectionsLoading ? (
        ...
        <div className={styles.setup}>
          ...
          {selectedConnection && <div className={styles.connectionContext}><div><span>Workspace connection</span><strong>{selectedConnection.name}</strong><small>{selectedConnection.status === "invalid" ? "Access expired" : "Used for provider resources"}</small></div><button type="button" disabled={!canAttach} onClick={() => void reconnectProvider()}>{selectedConnection.status === "invalid" ? "Reconnect access" : "Refresh authorisation"}</button></div>}
          <div className={styles.attachPanel}>
            ...
            {loading ? <p className={styles.state}>Loading available {copy.resourceLabel}s…</p> : options.length ? (
              ...
              <ProviderFailure provider={provider} message={error} code={errorCode} canChange={canAttach} onRetry={retryOptions} onReconnect={() => void reconnectProvider()} />
            ) : (
              <div className={styles.emptyState}>
                <strong>No {providerLabel(provider)} {pluralResource(copy.resourceLabel)} were returned.</strong>
                <span>{provider === "github" ? "This is a real empty result from GitHub, not a connection error. If the account should contain repositories, refresh the workspace authorisation." : `This connection currently has no accessible ${pluralResource(copy.resourceLabel)}.`}</span>
                <div><button type="button" onClick={retryOptions}>Retry</button>{provider === "github" && <button type="button" disabled={!canAttach} onClick={() => void reconnectProvider()}>Reconnect workspace GitHub</button>}</div>
              </div>
            )}
          </div>
        </div>
...
function ProviderFailure({ provider, message, code, canChange, onRetry, onReconnect }: { provider: Provider; message: string; code: string; canChange: boolean; onRetry: () => void; onReconnect: () => void; }) {
  const reconnectRequired = code === "github_reauthentication_required" || code === "github_access_forbidden";
  return (
    <div className={styles.providerFailure} role="alert">
      <span>{reconnectRequired ? "Workspace authorisation expired" : `${providerLabel(provider)} request failed`}</span>
      <strong>{message}</strong>
      <p>{reconnectRequired ? "This is the provider token stored for this workspace—not your InfraMap login or profile connection." : "The repository list was not empty; InfraMap could not retrieve it."}</p>
      <div><button type="button" onClick={onRetry}>Retry request</button>{reconnectRequired && <button type="button" disabled={!canChange} onClick={onReconnect}>Reconnect workspace GitHub</button>}</div>
    </div>
  );
}
...
async function loadProviderOptions(provider: Provider, accountID: string, connectionID: string): Promise<ProviderOption[]> {
  ...
  const projects = await getVercelServices(accountID, connectionID);
...
function ConnectedProviderResource({ provider, resource }: { provider: Provider; resource: WorkspaceResource; }) {
  const copy = providerCopy[provider];
  ...
  return (
    <section id={copy.sectionID} className={styles.section}>
      <header className={styles.heading}><div><p className={styles.eyebrow}>{copy.eyebrow}</p><h2>{resource.name}</h2><p>This {providerLabel(provider)} {copy.resourceLabel} is attached and ready for workspace operations.</p></div><span className={styles.providerMark}>{copy.mark}</span></header>
...
function stringMetadata(resource: WorkspaceResource, key: string, fallback: string): string {
...
function pluralResource(value: string): string {
  return value === "repository" ? "repositories" : `${value}s`;
}
