...
import { getSupabaseProjects } from "@/app/features/workspaces/connections";
import type { Connection, SupabaseProject } from "@/app/components/types/users";
import { attachSupabaseResource, createSupabaseResource } from "../api";
import type { ProjectResource, WorkspaceProjectDetail } from "../types";
import styles from "./SupabaseProjectStage.module.css";

type SetupMode = "attach" | "create";
type SupabaseOrganization = { id: string; slug: string; name: string };

type SupabaseProjectStageProps = {
  accountID: string;
  projectID: string;
...
export function SupabaseProjectStage({ accountID, projectID, role, resource, connections, connectionsLoading, connectionsError, onAttached }: SupabaseProjectStageProps) {
  ...
  useEffect(() => {
    ...
    void Promise.resolve().then(async () => {
      ...
      setError("");
      try {
        const available = await getSupabaseProjects(accountID, selectedConnection.id);
        if (active) {
  ...
  async function attachProject() {
    ...
    setError("");
    try {
      onAttached(await attachSupabaseResource(accountID, projectID, selectedConnection.id, selectedProjectRef));
    } catch (cause) {
  ...
  async function createProject(event: FormEvent<HTMLFormElement>) {
    ...
    setError("");
    try {
      const attached = await createSupabaseResource(accountID, projectID, {
        ...
        <div className={styles.setup}>
          ...
          {mode === "attach" ? (
            <div className={styles.attachPanel}>
              ...
              {projectsLoading ? <p className={styles.state}>Loading available projects…</p> : projects.length ? (
                <>
...
function ConnectedSupabase({ resource }: { resource: ProjectResource }) {
  ...
  return (
    <section id="data-layer" className={styles.section}>
      <header className={styles.heading}><div><p className={styles.eyebrow}>Data layer</p><h2>{resource.name}</h2><p>This Supabase project is attached to the InfraMap project and ready for its next operational controls.</p></div><span className={styles.providerMark}>S</span></header>
